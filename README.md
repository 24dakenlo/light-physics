...
前述のように, 物質の誘電率は正弦的に振動する電場の振動数（角速度）に依存する。そして光（がもたらす電場）の振動数は光の波長に依存する。従って, 屈折率は光の振動数や波長に依存する。これを光の分散dispersionという。たとえば水の屈折率は可視光域でも紫色光($0.4 \micro\text{m}$)の1.3310から赤色光($0.7 \micro\text{m}$)の1.3390まで変化する。わずか0.6 %の変化だが, 水滴に入射する日光はこれによって経路が分かれ, 異なった角度で水滴から出ることによって, 空に虹ができるのである。
