...
前述のように, 物質の誘電率は物質の物性にも依存する。とりわけ, 結晶のように構造が強い異方性を持つ物質では, 特定方向の誘電率が大きかったり小さかったりする。これは, 屈折率が方向によって違うということである。つまり, 電場の方向によって屈折率は異なる値を取り得る。そのような物体に入射する光は, 特異的な方向の直線偏光に分解して考えると, ある方向の直線偏光は遅く, 別の方向の直線偏光は速く伝わる。これを複屈折という。複屈折を利用する生態系現象は著者には思いつかない。むしろ複屈折は計測技術に有用である。液晶技術はこの応用例である。直線状の高分子の配向を電気的に操作して偏光の透過率を変えるのだ。それによって非常に高速に作動するシャッターができる。生物体で短時間に起きる現象を光学的に観察する際にこの技術を使う人もいるだろう。自然界の物質で複屈折が顕著なのは方解石である。方解石とその複屈折は微分干渉顕微鏡（DIC）に使われる。光源から方解石板に入射した光は複屈折によって微妙に異なる2つの角で屈折するため, わずかにずれた2つの偏光に分かれる。それがサンプルを通過した後, 再度, 方解石板によってひとつに戻すと, サンプルの厚みに対応する光路差の違いが強調される。これによって, 普通の光学顕微鏡では観察しにくいような透明なサンプルも鮮明に観察できるのである。方解石以外の鉱物や氷結晶も複屈折を呈する。そのため, それらを薄いサンプルにして偏光で観察することで結晶構造を鮮明に可視化できる。

媒質の中の偏光に触れたついでに, 「光学活性」についても言及しておこう。これは, キラル化合物の片方のエナンチオマーだけからなる溶液に直線偏光が入射した時, 光が進むにつれて偏光面が回転していくという現象である。単糖類や多くのアミノ酸は光学活性を持つ。
