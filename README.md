...
今, 単純化のために, 等しい大きさ $a$の水平な葉が樹冠内に一様ランダムに分布しており, 葉以外の構造物（枝, 幹など）は無視できるとする。単位体積中に葉は $n$枚あるとする。この林冠に, 細い直線状の光が上から注ぐとする。 $x$は群落表面からの距離とする。森林の面積を $A$とすると, この光はその中のどこかに落ちようとしている。
...
$I(x+\Delta x)-I(x)=-an I(x) \Delta x$

となる。両辺を $\Delta x$で割り, $\Delta x$を十分に0に近づけると（注: $\lim_{\Delta x \rightarray 0} \frac{I(x+\Delta x)-I(x)}{\Delta x} = \frac{dI}{dx}$）, $an$を $\beta$と書くと, 

$\frac{dI}{dx} = -\beta I(x)$

となる。もし $\beta$が$x$によらない定数なら（つまり葉の大きさと数密度の積, つまり葉面積密度が群落の深さによらず一定なら）, この微分方程式は簡単に解けて, 

$I(x) = I(0) \exp (-\beta x)$

となる。この理論は適用範囲が広いのだが, 限界もある。それを確認するには導出過程に遡る必要がある: まず, 葉に当たった光は完全に遮られるものとされた。つまり, 葉は光を吸収またはそのまま透過するだけであり, 散乱（反射）しないことが前提である。次に, 葉が空間的に一様にランダムに分布している。つまり粗密は無いことが前提である。この前提は実は2つに分かれている: 1つめは水平方向に粗密が無いこと。これによって層状の空間を進む光の微分方程式が立てられた。2つめは深さ方向に粗密が無いこと。これによって微分方程式が解かれて上記のような指数関数が得られた。これら3つの前提がどれだけ満たされるかによって, この法則の適用範囲が決まるのである。
...
となる。さて, もし光がこの森林の真上から降り注ぎ, しかも葉に当たった光は完全に吸収されるなら, 明らかにこの樹冠の透過率はこの $t$に等しいはずだ。つまり, 森林表面から下向きに $x$軸をとり, 位置 $x$におけける光の強さを $I(x)$とすると, 光の透過率は $I(x)/I(0)$だから, 

$I(x)/I(0) = t = \exp(-L)$

ここで, $L$は単位面積当たりの上空の葉面積だが, 樹冠の深さ（高さ）を$h$として
