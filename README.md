...
先に述べたように、人間の視覚は波長が約 400 nm 未満または約 700 nm以上の光を感知できない。これは、人間の目の錐体がそのような光に反応しないためである。つまり, 人間とは異なる種類の錐体を持つ動物は、人間には見えない光を見ることができるのだ。実際、鳥や一部の昆虫は人間の可視光より波長の短い紫外線（UV）に反応する錐体を持っている。そのため、これらの動物の視覚を研究する際には、私たちにとって見えない光を考慮する必要がある。
...
光が媒質（森林樹冠のような構造物や, エアロゾルが充満した大気など, 光を遮る少物体や粒子が空間内に分布した状況）を通貨するとき, 距離が進むにつれて光は減衰していく。その様子の最も単純なモデルを説明する。それはLambert Beerの法則（注: Beer Lambert's law, Bouguer-Lambert-Beer’s lawなどとも呼ばれる）というものである。すなわち, 光の進んだ距離を $x$とし, そこでの光強度を $I(x)$とすると, 

$I(x) = I(0) \exp(-x/\tau)$

と表せる。ここで $\tau$は光学的厚さと呼ばれる定数であり, 媒質や構造, 入射角などによって決まる。これを単純な森林樹冠における光の透過について導出してみよう。

今, 単純化のために, 等しい大きさ $a$の水平な葉が樹冠内に一様ランダムに分布しており, 葉以外の構造物（枝, 幹など）は無視できるとする。林の中に面積 $A$の地表を考え, その上空にある葉の数を $N$とする。$この地表を上空から見ると, 葉の隙間から地表が見える面積は, 葉の数 $N$が多いほど小さい。葉が1枚のとき($N=1$)は, 面積 $A$の中で面積 $a$だけ葉に覆われるため, 隙間の面積の割合 $t$は（注: これが最終的に透過率になる）, 

$t = (A-a)/A=1-a/A$

である。葉の分布が一様ランダムであることから, 葉は互いに独立に地表を覆うので, $N$枚の葉をもってしても覆われない地表, つまり隙間の面積の割合 $r$は, 上式の $N$乗, つまり

$t = (1-a/A)^N$

となる。ここで, この面の上空にある葉の総面積 ($Na$)を地表面積 ($A$)で割った値（地表面積の何倍の面積が樹冠にあるか）を葉面積指数と呼び, ここでは$ L$と表そう:

$L = Na/A$

すると $a/A=L/N$となり, 上の式は, 

$t = (1-L/N)^N$

となる。ここで$N$が十分に大きい（個々の葉の面積 $a$は十分小さい）とみなせば, 数学の公式
...
この法則は, 光が遮られるときに吸収される場合は良く成り立つ。そうでない場合, つまり光が散乱されることが無視できない場合は, Kubelka-Munk理論などによって修正される。
