...
さて, 光が真空でない場所, すなわち水やガラスなどの媒質・物質を伝播する場合を検討しよう。媒質の中には無数の電子や原子核などが前述のMaxwell方程式の中の $\rho$や $\mathbf{J}$に関与してくるので, Maxwell方程式を直接扱うことは大変むずかしい。そこで, それらの媒質を構成する物質たちが作る電荷や電流を, ある仮定のもとに巨視的にモデル化することで, 媒質中の巨視的な電磁気現象を扱えるように工夫されたのが, 媒質中のMaxwell方程式と呼ばれるものである。それは上の式の中の第一方程式と第四方程式をそれぞれ以下のように書き換えたものである（第二, 第三方程式はそのまま生きる）:
