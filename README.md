...
$\mathbf{D} = \epsilon\mathbf{E}$　　　　(5-1)

$\mathbf{B} = \mu\mathbf{H}$　　　　(5-2)
...
さて, 奇妙なことに, $\epsilon$, $\mu$は複素数, つまり虚数部を持つ量になり得る。物理学のように現実の現象を扱う学問で「虚数」が現れることは多くの学生を戸惑わす。しかしそこまで深く考えることはなく, これは数式の便宜的な表現の結果である。すなわち, 正弦波的に振動する電場$\mathbf{E}$が入ってくれば, 媒質中の電荷も正弦的な力を受け, 正弦的に運動する。その結果, 電束密度$\mathbf{D}$も正弦的に振動する（注: それには「電気双極子モーメント」の振動が介在するが詳細は割愛する。また, 非線形光学材料と呼ばれる特殊な物質ではその限りではない。なお, 磁場$\mathbf{H}$の変動についても割愛する）。ただし, $\mathbf{D}$の振動のタイミング（位相）は$\mathbf{E}$のそれと同期しているとは限らず, わずかに遅れることがある。その遅れを含めて, 式(5-1)のように$\mathbf{D}$と$\mathbf{E}$を比例関係で表すには, 複素数が必要になる。つまり, 振動の位相の遅れを$\delta$とすると（通常, $\delta$は0に近い）, $\epsilon$は複素数平面上で$-\delta$という偏角を持つ複素数とすればよい。すなわち, 誘電率$\epsilon$は以下のように実部$\epsilon'$と虚部$-\epsilon''$を持つ複素数となる（虚部にマイナスをつけるのは, $\mathbf{D}$は$\mathbf{E}$よりも遅れるということを意味する。これによって$\epsilon''$は正の実数になる）:

$\epsilon = \epsilon' - i \epsilon''$　　　(5-3)

$\tan \delta = \epsilon''/\epsilon'$　　　(5-4)

上の性質3で分母の $\epsilon \mu$に絶対値をつけたのは, そのためである（光速は虚数部を持つことは無い）。
