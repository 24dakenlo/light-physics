...
$I(x+dx)-I(x)=-beta I(x) dx$



し, 光の単位面積あたりの強さを $I(x)$とする。

光が微小距離 $dx$だけ群落を（下に向かって）進むと, 体積 $Adx$の微小空間（薄い板状の空間）内に存在する葉が光の一部を遮る。その微小空間に存在する葉の数は　$nAdx$だから, それらの葉の合計面積は $naAdx$である。


林の中に面積 $A$の地表を考え, その上空にある葉の数を $N$とする。$この地表を上空から見ると, 葉の隙間から地表が見える面積は, 葉の数 $N$が多いほど小さい。葉が1枚のとき($N=1$)は, 面積 $A$の中で面積 $a$だけ葉に覆われるため, 隙間の面積の割合 $t$は（注: これが最終的に透過率になる）, 
