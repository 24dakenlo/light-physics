...
となる。両辺を $\Delta x$で割り, $\Delta x$を十分に0に近づけると（注: $\lim_{\Delta x \rightarray 0} \frac{I(x+\Delta x)-I(x)}{\Delta x} = \frac{dI}{dx}$）, 
...
となる（注: ここでは微分方程式を解いたが, 別の方法でも(LB05)を導出できる。(LB02)から, 
$I(x+\Delta x) = (1 - an \Delta x) I(x)$である。群落を表面($x=0$)から深さ$x$までを, 厚さ $\Delta x$の$N$枚の水平層に分割すれば(x=N\Delta x$), 各層を通過すたびに光はこの式に従って弱まっていく。つまり, 
...
$I(x) = I(0) \exp (-\int_0^x \beta dx)$　　(LB05)
